...
- Each public method except `start` and `shutdown` enqueues a private callable and blocks on `concurrent.futures.Future.result()`; the original exception is re-raised in the caller. Requests are serialized by one queue. `start`, `close_source`, and `shutdown` are idempotent; a repeated `shutdown` is a no-op, while every other call after shutdown raises `RuntimeError`.
- `SourceHandleInfo` is a frozen dataclass with `source`, `signature`, and `sheets` and contains no COM object.
- `save_plain_master` receives an existing app-owned unique run directory, creates and owns `m.xlsx` inside it, and returns that file path on success. On SaveAs or verification failure it closes the source and attempts to delete the master; deletion failure reports the plaintext path. The caller always owns final run-directory cleanup and retry.

- [x] **Step 1: Write failing thread-affinity and lifecycle tests**
...
- [x] **Step 2: Run the source session tests and confirm RED**
...
- [x] **Step 3: Implement the command worker and source lifecycle**

Use a non-daemon `threading.Thread`, a request `queue.Queue`, and response `Future` objects. The worker creates the Excel session inside its own thread. `call` enqueues plain arguments and returns plain dataclasses. Capture the full source signature before open. Before inspect and snapshot, reject size or mtime changes with a quick stat; immediately before master save, recalculate and compare the full signature. `shutdown` is idempotent and joins the worker.

- [x] **Step 4: Implement source operations with shallow inspect**
...
- [x] **Step 5: Implement plain master SaveAs and verification**
...
- [x] **Step 1: Write failing scalar, matrix, error, order and Text-call tests**
...
- [x] **Step 3: Implement value normalization and representative Text reads**
...
- [x] **Step 4: Run classifier and snapshot tests**
...
- [x] **Step 1: Write failing scheduling, concurrency and failure tests**
...
- [x] **Step 3: Implement the bounded dynamic queue**
...
- [x] **Step 4: Implement fatal partial-result propagation and cleanup**
...
- [x] **Step 1: Write failing integration-level unit tests**
...
Run: `python -m pytest tests/unit/test_controller.py tests/unit/test_excel_gateway.py tests/unit/test_split_service.py -q -p no:cacheprovider`

Expected: failures show the old per-action Excel sessions and serial writer.

- [x] **Step 3: Wire source session and bulk snapshot behavior**
...
- [x] **Step 7: Rebuild and verify the EXE**

Run: `powershell -ExecutionPolicy Bypass -File scripts/build.ps1`

Verified: dependency checks, `118 passed`, one-folder startup probe, one-file build, embedded `pythoncom` DLL inspection and final EXE pywin32 self-test all passed. Output: `dist/ExcelSplitter.exe`, 12,834,638 bytes, SHA-256 `56FE1C8CE4DD6AB9E82BCE029E1DAB00F719202F8D49F413CAD98BDD765D9CBC`.

- [x] **Step 8: Record release limitations**
